...
//6.1下载express-session 并配置
const session =require('express-session');
app.use(session({
	secret:'128位随机字符',//安全字符串
	resave:false,//每次请求是否都更新数据
	saveUninitialized:true,//初始化时数据
	cookie:{
		maxAge:1000*60*60*8//保存时间8小时
	}
}));
...
// 3.返回数据结果json
	...
	// 	  if(err)throw err;
	// 	  if(result.length==0){
	// 		res.send({code:0,smg:"登录失败"});
	// 	  }else{
	// 		//将用户登录凭证保存在服务器端session中
	// 		var id=result[0].id;//获取当前用户id
	// 		req.session.uid=id;//保存在session中
	// 		console.log(req.session.uid)
	// 		res.send({code:1,smg:"登录成功"});
	// 	  }
	//    })
	//   })
